...
## 규칙 17: 집계 차트는 항상 대구분 단위
도넛/막대/카테고리 분포 등 집계 차트는 **대구분(category) 단위만 사용**, 개별 task_label 은 그룹키로 쓰지 않는다.
- daily_work_entries POST 시 분류 정보(`task_category` 라벨 또는 `task_category_id`) 빠지면 안 됨
- 서버 집계 함수(`computeByType` 등) 의 fallback 은 `'기타'` 또는 source 기반 라벨까지만. task_label 절대 금지.
- 새 입력 화면 만들 때: daily_work_entries 에 분류 라벨 전달하는지 코드 리뷰 (DATA_SYNC_RULES §3 버그 J)

## 규칙 16: 로컬 편집 상태와 서버 캐시 분리 시 화면 즉시 반영
한 화면에서 **본인 편집 데이터(로컬 변수)** 와 **다른 사용자 데이터(서버 캐시)** 를 함께 표시할 때:
- 렌더 함수는 본인은 **로컬 상태(`chkDone` 등)**, 타인은 **서버 캐시(`allMembersWorkTasks` 등)** 를 사용해야 함
- 로컬 변경을 일으키는 모든 액션(토글·취소·초기화·저장)이 **즉시 화면 재빌드 함수**를 호출하도록 점검 (DATA_SYNC_RULES §3 버그 I)
- "저장 시 fetch 재렌더만으로 충분" 가정 금지 — 사용자는 저장 전부터 반영을 기대함
- 체크리스트·캘린더·상태표 등 토글 가능한 UI에 작업할 때 반드시 이 패턴 확인

## 규칙 15: 사용자가 입력한 시간/날짜 강제 변환 금지
직원이 입력한 `start_time`, `end_time`, `start_date`, `end_date` 는 **그대로 저장**한다.
시스템이 09:00, 18:00 같은 기본값으로 임의 대체 금지.
- 멀티데이 분산: 각 일자에 **사용자 입력 시간을 동일 적용** ("매일 같은 시간 근무" 모델).
  옛 "첫날 ~ 18:00 / 09:00 ~ 마지막날" 비대칭 변환은 폐기 (DATA_SYNC_RULES §3 버그 H).
- 자동 보정 가능 항목은 **분(minutes)** 같은 파생값뿐. 그것도 명확 정책(점심 12:00~13:00 1h 차감) 하에서만.
- 표준 근무시간: 09:00~18:00 (점심 제외 = 8h/day). `server/lib/timeentry.js` `netMinutesInRange()` 가 단일 SoT.
- 클라이언트가 보낸 `minutes` 값은 신뢰하지 않고 서버에서 항상 재계산 (`normalizeTimeEntries`).
- 데이터 동기화 흐름 작업 시 반드시 **`docs/DATA_SYNC_RULES.md` §3 버그 H + 시간 정합성 정책 요약 표** 참조.

## 규칙 18: 코드 반영 시 반드시 "유저 기반 실동작 테스트" 후 보고
기능을 만들거나 고친 뒤에는 **코드 리뷰·API 200 확인만으로 "됐다"고 보고하지 않는다.**
실제 사용자가 하는 행동 그대로를 재현해 **동작이 되는지 눈으로 확인한 근거**와 함께 보고한다.

### 18-1. 최소 검증 절차 (저장/입력/집계 등 상태변경 기능)
1. **서버 API 종단 테스트**: 실제 엔드포인트에 요청 → **재조회(GET)로 영속 확인** → **디스크 파일(`portal-db.json`)에 실제 기록됐는지**까지 확인.
   (요청이 200 이고 응답 body 에 저장건수가 있어도 `writeDb()` 누락 등으로 **디스크 미반영**일 수 있음 — 2026-09 `/weekly/bulk-save` 사례)
2. **브라우저 UI 테스트**: Playwright(`tmp/node_modules/playwright-core`, chromium=`/opt/pw-browsers/...headless_shell`)로
   세션 주입 → 화면에서 **입력 → 저장 버튼 클릭 → 새로고침 → 값 유지**를 실제로 재현.
3. **동시성 테스트**: 2인 이상(직원A·직원B) 동시 저장 → **서로 덮어쓰지 않고 각자 보존**되는지 확인.
4. 테스트는 **시드 DB 사본**에 대해 수행한다 (`DATA_PATH` 환경변수로 사본 지정). 실 DB/시드 훼손 금지.

### 18-2. 테스트 격리 규칙
- 서버는 자식 프로세스로 띄우고(백그라운드 node 는 SIGTERM 당함) `PORT`/`DATA_PATH` 를 env 로 분리.
- 검증 스크립트는 `tmp/_check_NN_*.js` (규칙 3). 실행 후 임시 DB 는 삭제.
- **textarea/select 값은 `innerText` 로 확인 금지** — `.value` 로 읽는다 (오탐 주의).

### 18-3. 보고 형식 (사용자에게 반드시 이렇게 보고)
- "됐다/완료" 대신 **무엇을·어떤 시나리오로·몇 건 통과**했는지 표로 제시.
- **테스트 안 한 부분은 "안 했다"고 명시**한다 (예: Windows PowerShell 은 이 환경에서 실행 불가 → 로컬 검증 필요).
- 실패를 발견하면 숨기지 말고 원인·수정·재검증 결과까지 함께 보고.

### 18-4. 배포 반영 주의
- **`server/*.js` 수정은 파일 pull 만으로 반영 안 됨** — 실행 중 node 프로세스 **재시작 필요**.
  auto-pull 스크립트가 `server/` 변경 시 자동 재시작하는지 확인하고, 안내에 재시작 필요 여부를 명시한다.
- HTML/CSS/JS 정적 파일은 재시작 불필요(서버가 디스크에서 서빙). 단 `?v=` 캐시버스트는 규칙 14-3 준수.
